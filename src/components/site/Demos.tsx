...
export const Demos = () => {
  ...
  const renderCard = (d: DemoItem, i: number) => (
    <article key={d.slug} className="reveal-item">
      <Link
        to={`/demos/${d.slug}`}
        className="demo-card group relative block aspect-[2/3] overflow-hidden rounded-[12px] border border-white/[0.08] hover:border-[rgba(252,163,17,0.25)] transition-colors duration-200"
      >
        <img
          src={d.img}
        ...
        >
          <div className="min-w-0 flex-1">
            <span
              className="inline-block font-mono text-[13px] uppercase tracking-widest text-[#FCA311] border border-[#FCA311] px-5 py-[10px]"
              style={{ borderRadius: 2 }}
            >
              {d.pkg}
            </span>
            <h3 className="font-display text-lg leading-tight text-white mt-5 truncate">
              {d.brand}
            </h3>
            <p className="font-mono text-[14px] uppercase tracking-wider text-white/50 mt-[2px] truncate">
              {d.desc[lang]}
            </p>
          </div>
          <div className="flex flex-col gap-2 shrink-0">
            <span
              className="font-mono uppercase text-[12px] tracking-wider text-white border border-white/20 hover:border-white/60 transition-colors px-[14px] py-[10px] text-center"
              style={{ borderRadius: 2 }}
            >
              {lang === "es" ? "Ver demo" : "View demo"}
            </span>
            <span
              className="font-mono uppercase text-[12px] tracking-wider text-black bg-[#FCA311] px-[14px] py-[6px] text-center"
              style={{ borderRadius: 2 }}
            >
    ...
    <section id="demos" className="glow-section noise-overlay bg-carbon text-bone py-24 md:py-36 relative">
      <div ref={ref} className="container-editorial reveal-up">
        <div className="mb-16 md:mb-20 text-center">
          ...
          <p className="font-display text-4xl md:text-6xl lg:text-7xl leading-[0.95] text-balance mt-8">
            {t("demos.body")}
          </p>
        </div>
        ...
        {/* Grid expandido — justo debajo del inicial */}
        <div
          id="showroom-expanded"
          className="showroom-expand grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-4 md:mt-6"
          data-open={expanded}
        ...
        {/* Botón ver más — siempre entre grids y CTA */}
        ...
        {/* CTA principal — siempre al final */}
